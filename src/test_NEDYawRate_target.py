...
# ===================== WAYPOINT (EDIT HERE) =====================
WAYPOINT_LAT = 14.7055816
WAYPOINT_LON = 101.5730007
WAYPOINT_ALT_REL = 5.0   # meters relative altitude
# ================================================================
...
async def main():
    ...
    print("\n=== WAYPOINT FOLLOW (DEBUG) ===")
    print("1) REMOVE PROPS for testing")
    print("2) ARM")
    print("3) Drone will fly to hard-coded waypoint")
    ...
    while True:
        ...
        if armed:  # and enabled
            # --- Horizontal ---
            dist, bearing_rad = distance_and_bearing(
                lat, lon, WAYPOINT_LAT, WAYPOINT_LON
            )
            ...
            # --- Vertical ---
            alt_error = WAYPOINT_ALT_REL - rel_alt
        ...
        if now - last_print > 0.5:
            ...
            print(f"  GPS pos: lat={lat:.6f} lon={lon:.6f} alt={rel_alt:.2f}")
            print(f"  Distance to WP: {dist:.2f} m")
            print(f"  Bearing to WP: {math.degrees(bearing_rad):.2f} deg")
